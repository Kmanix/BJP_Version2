...
SystemController::SystemController(long int& cr, TrafficGen* tg, StatKeeper* sm,
	vector<Player*>& pb, vector<Dealer*>& db, vector<GameTable*>& tb,
	vector<Player*> qp, vector<Player*> hf)

	: cashReserve(cr), trafficGenModule(tg), statModule(sm),
	initialPlayerBase(pb), dealerBase(db), casinoTables(tb),
	quitPlayers(qp), hallOfFame(hf)
{
